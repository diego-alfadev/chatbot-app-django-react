...
@permission_classes([IsAuthenticated])
@authentication_classes([TokenAuthentication])
def user_message(request: Request):

    user = request.user
    # get the most recent chat ordered by created_at
    chat = Chat.objects.filter(user=user).order_by('-created_at').first()

    # if there is no chat or the most recent chat has more than 1 day, create a new chat
    if not chat or (chat.created_at - timezone.now()).days > 1:
    ...
    return Response({"response": MessageSerializer(response).data, "chat": ChatSerializer(chat).data})
